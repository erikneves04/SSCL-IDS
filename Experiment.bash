...
python3 evaluation.py --test_dataset_name "$DATASET_PATH" --batch_size "2046" --model_chkpt "$RENAMED_CHKPT" --train_from_scratch False >> $TERMINAL_LOGS_FILE 2>&1 #--results_file "$RESULTS_FILE"
echo "#################################################################################" >> $TERMINAL_LOGS_FILE

# Avaliação supervisionada
echo "Starting supervised evaluation..." >> $TERMINAL_LOGS_FILE
python3 evaluation_supervised.py --supervised_training_dataset "$DATASET_PATH" --batch_size "2046" --model_chkpt checkpoints/"$RENAMED_CHKPT" --supervised_model "$SUPERVISED_MODEL" >> $TERMINAL_LOGS_FILE 2>&1 #--results_file "$RESULTS_FILE"
echo "#################################################################################" >> $TERMINAL_LOGS_FILE
